<?php
    	...
    	if($errflag) {
    		...
    		session_write_close();
    		header("location: loginresc.php");
    		exit();
    	}
     
    	//Create query
    	$qry="SELECT * FROM rescue WHERE username='$username' AND password='$password'";
    	...
    	if($result) {
    		if(mysqli_num_rows($result) > 0) {
    			...
    			session_write_close();
    			header("location: inbetweenresc.php");
    			exit();
    		}else {
    			//Login failed
    			...
    			if($errflag) {
    				$_SESSION['ERRMSG_ARR'] = $errmsg_arr;
    				session_write_close();
    				header("location: https://offthegrid.000webhostapp.com/hdoc/inbetweenresc.php");
    				exit();
    			}
    		}
    	}else {
